...
def pace_forecast_month(df: pd.DataFrame, cutoff: pd.Timestamp, start: pd.Timestamp, end: pd.Timestamp, ref_years: int = 2, dmax: int = 180, props: Optional[List[str]] = None, inv_override: Optional[int] = None) -> dict:
    ...
    Devuelve dict con: nights_otb, nights_p50, nights_p25, nights_p75, adr_final, revenue_final, details(DataFrame con por-día)."""
    ...
    # ADR del remanente aproximado desde ref: usar D_mediano
    D_med = int(np.median(D_day))
    # construir ADR_tail median a D_med desde refs
    tail_adrs = []
    for k in range(1, ref_years+1):
        ...
        if base.empty:
            continue
        nights_final = base.loc[base['D']==0, 'noches'].values[0]
        rev_final = base.loc[base['D']==0, 'ingresos'].values[0]
        nights_atD = base.loc[base['D']==D_med, 'noches'].values[0] if D_med in base['D'].values else np.nan
        rev_atD = base.loc[base['D']==D_med, 'ingresos'].values[0] if D_med in base['D'].values else np.nan
        dn = max(nights_final - nights_atD, 0.0)
        dr = max(rev_final - rev_atD, 0.0)
        if dn > 0:
            tail_adrs.append(dr/dn)
    adr_tail = float(np.median(tail_adrs)) if tail_adrs else 0.0
    ...
    nights_rem = max(nights_p50 - nights_otb, 0.0)
    if nights_p50 > 0:
        revenue_final = rev_otb + adr_tail * nights_rem
        adr_final = revenue_final / nights_p50
    else:
        revenue_final = 0.0
        adr_final = 0.0
...
# =============================
    ...
    if run_f:
        ...
        adr_final = res['adr_final']; rev_final = res['revenue_final']

        c1, c2, c3 = st.columns(3)
        c1.metric("OTB Noches", f"{nights_otb:,.0f}".replace(",","."))
        c2.metric("Forecast Noches (P50)", f"{nights_p50:,.0f}".replace(",","."))
        c3.metric("Forecast Ingresos (P50)", f"{rev_final:,.2f}")
        c4, c5 = st.columns(2)
        c4.metric("ADR final (P50)", f"{adr_final:,.2f}")
        c5.metric("Banda Noches [P25,P75]", f"[{nights_p25:,.0f} – {nights_p75:,.0f}]".replace(",","."))

        # Gráfica: OTB acumulado vs líneas horizontales de forecast P25/P50/P75
        daily = res['daily'].copy()
        daily['OTB acumulado'] = daily['noches_ocupadas'].cumsum()
        horiz = pd.DataFrame({
            'OTB acumulado': [np.nan, np.nan],
            'P25 final': [nights_p25, nights_p25],
            'P50 final': [nights_p50, nights_p50],
            'P75 final': [nights_p75, nights_p75],
        }, index=[daily['Fecha'].min(), daily['Fecha'].max()])
        plot_df = pd.concat([daily.set_index('Fecha')[['OTB acumulado']], horiz], axis=0)
